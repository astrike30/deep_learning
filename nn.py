...
class NeuralNetModel:
    ...
    def __init__(self, noInputs, noHiddens, noOutputs, lr):
        ...
        self.weights.append(np.random.normal(0.0, pow(noHiddens[0], -0.50),
                                (noHiddens[0], noInputs)))
        self.biases.append(np.zeros((noHiddens[0], 1)))
        for i in range(1, len(noHiddens)):
            self.weights.append(np.random.normal(0.0, pow(noHiddens[i], -0.50), 
                                (noHiddens[i], noHiddens[i-1])))
            self.biases.append(np.zeros((noHiddens[i], 1)))

        self.weights.append(np.random.normal(0.0, pow(noOutputs, -0.50), 
                            (noOutputs, noHiddens[-1])))
    ...
    def train(self, inputs, targets):
        ...
        for i in reversed(range(0, len(self.noHiddens) + 1)):
            self.weights[i] += self.lr * np.dot((output_error * outputs[i+1] * (1.0 - outputs[i+1])), 
